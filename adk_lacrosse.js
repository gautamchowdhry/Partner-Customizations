    ...
    if(url.indexOf('/registration?bid=') > -1){ 
      ...
      if (user_id) {
        var headers = {
          ...
          'x-api-key': '9e82cf433d5555a1ad29db8daddf8d47'
          };
        ...
        $.ajax(site_url + '/users/' + user_id + '/registrations', {
          ...
          headers: headers
           ...
           // 2nd child and higher
            if (regCount >= 1) {
              var newText = 'Congratulations. You qualify for our <strong>multi player sibling discount</strong>.  You\'ll <strong>$50</strong> on this registration.'
              $('[data-id="discount-code"]').after('<div class="participant-headsup">' +newText + '</span>');
              $('#discountCode').val('adksib50');
