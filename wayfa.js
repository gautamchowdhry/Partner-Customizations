    ...
    if(url.indexOf('/registration?bid=') > -1){ 
      ...
      if (user_id) {
        var headers = {
          ...
          'x-api-key': 'ffb34ccf8586299fa2c227ac9d1e880e'
          };
        ...
        $.ajax(site_url + '/users/' + user_id + '/registrations', {
          ...
          headers: headers
            ...
            if (regCount >= 1) {
              var newText = 'Congratulations. You qualify for our <strong>multi player sibling discount</strong>.  You\'ll save <strong>$40</strong> on this registration.'
              $('[data-id="discount-code"]').after('<div class="participant-headsup">' +newText + '</span>');
              $('#discountCode').val('wayfafamily');
