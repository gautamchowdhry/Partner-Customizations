    ...
    if(url.indexOf('/registration?bid=') > -1){ 
      ...
      if (user_id) {
        var headers = {
          ...
          'x-api-key': '3f65c6153e3eb3e94f8d53bbb76d051c'
          };
        ...
        $.ajax(site_url + '/users/' + user_id + '/registrations', {
          ...
          headers: headers
            ...
            if (regCount >= 1) {
              var newText = 'Congratulations. You qualify for our <strong>multi player sibling discount</strong>.  You\'ll <strong>save $225</strong> total on this registration.'
              $('[data-id="discount-code"]').after('<div class="participant-headsup">' +newText + '</span>');
              $('#discountCode').val('MULTI');
