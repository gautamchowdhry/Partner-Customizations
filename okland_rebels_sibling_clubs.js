        ...
        $.ajax(site_url + '/users/' + user_id + '/registrations', {
          ...
          headers: headers
            ...
            if (pageTitle.toLowerCase().indexOf("club") > -1) {
                ...
                if (regCount == 1) {                
                  var newText = 'Congratulations. You qualify for our <strong>multi player sibling discount</strong>.  You\'ll <strong>save $200</strong> on this registration.'
                  $('[data-id="discount-code"]').after('<div class="participant-headsup">' +newText + '</span>');
                  $('#discountCode').val('Oakreb2ndchilddisc');
                 ...
                 if (regCount >= 2) {                
                  ...
                  $('[data-id="discount-code"]').after('<div class="participant-headsup">' +newText + '</span>');
                  $('#discountCode').val('Oakrebel3rdchilddisc');
                  $("#discountCode").attr('readonly', true);
                  $("#discountCode").attr("style", "background: #CCC; color: #333;border: 1px solid #666");                       
                }
