    ...
    if(url.indexOf('/registration?bid=') > -1){ 
      ...
      if (user_id) {
        var headers = {
          ...
          'x-api-key': '833a3d6dbcdb881e9a5544a4aaf897e6'
          };
            ...
            if (pageTitle.toLowerCase().indexOf("club") > -1) {
               ...
               //2nd registration
                if (regCount == 1) {
                  ...
                  $('#discountCode').val('Mustanghockeyassoc2ndand3rd');
               ...
               //4th and higher
              if (regCount >= 3) {
                  var newText = 'Congratulations. You qualify for our <strong>multi player sibling discount</strong>. This registration <b>is free</b>.'
                  $('[data-id="discount-code"]').after('<div class="participant-headsup">' +newText + '</span>');
                  $('#discountCode').val('Mustanghockey4thplus');
                  $("#discountCode").attr('readonly', true);
                  $("#discountCode").attr("style", "background: #CCC; color: #333;border: 1px solid #666");                       
                }
