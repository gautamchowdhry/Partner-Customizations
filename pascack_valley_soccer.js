    ...
    if(url.indexOf('/registration?bid=') > -1){ 
      ...
      if (user_id) {
        var headers = {
          ...
          'x-api-key': '2b30f6b7b039811976f6a7c2ded72c09'
          };
          ...
          headers: headers
            ...
            for (var i = 0, len = data.length; i < len; i++) {
              ...
              if (reg.programState != 'COMPLETED' && reg.userType == 'CHILD') {                
                if (reg.programId != '50339') // exclude preK program from counts
                    regCount++;                
                }
                                           }
           }
           ...
           //2nd registration
            if (regCount == 1) {
              ...
              $('#discountCode').val('2ndsibling');
              $("#discountCode").attr('readonly', true);
              $("#discountCode").attr("style", "background: #CCC; color: #333;border: 1px solid #666");                       
            }
            
            //3rd snd higher registration
            if (regCount >= 2) {
              var newText = 'Congratulations. You qualify for our <strong>multi player sibling discount</strong>.  You\'ll <strong>save $75</strong> on this registration.'
              $('[data-id="discount-code"]').after('<div class="participant-headsup">' +newText + '</span>');
