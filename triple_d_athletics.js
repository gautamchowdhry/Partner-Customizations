    ...
    if(url.indexOf('/registration?bid=') > -1){ 
      ...
      if (user_id) {
        var headers = {
          ...
          'x-api-key': 'db296026a397cbee4276843a55ce7ffa'
          };
        ...
        $.ajax(site_url + '/users/' + user_id + '/registrations', {
          ...
          headers: headers
            ...
            for (var i = 0, len = data.length; i < len; i++) {
              var reg = data[i];                            
              if (reg.programState != 'COMPLETED' && reg.userType == 'CHILD' && (reg.programType == 'TOURNAMENT' || reg.programType == 'LEAGUE' || reg.programType == 'CAMP')) {                
                regCount++;                
              }
           }
           ...
           // 2nd child and higher
            if (regCount >= 1) {
              ...
              $('#discountCode').val('Tripledsibdisc20');
